...
def remove_logger(logger_name, fh=None):
...
def calculate_psnr(img1, img2):#sr gt
...
def calculate_mask_psnr(img1, img2, mask):
    # img1 and img2 have range [0, 255]
    img1 = img1.astype(np.float64)
    img2 = img2.astype(np.float64)
    ...
    img1 = img1*mask
    img2=img2*mask
    mse= np.sum((img1 - img2) ** 2)/np.sum(mask)/3
    if mse == 0:
        return float('inf')
 
    return np.mean(np.sqrt(np.sum((img1 - img2) ** 2, axis=2)))/3, 20 * math.log10(255.0 / math.sqrt(mse))
