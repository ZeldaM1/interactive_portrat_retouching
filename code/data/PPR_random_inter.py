...
class PPR_random_inter_dataset(data.Dataset):
    def __init__(self, opt):
        super(PPR_random_inter_dataset, self).__init__()
        self.opt = opt
        ...
        self.paths_MASK,  self.MASK_choices = util.get_random_MASK_paths(self.paths_GT, opt['json'], opt['inst_path'], opt['dataroot_MASK'], opt['multi_ins_p'], opt['single_ins_p'])
        ...
        if self.paths_LQ and self.paths_GT:
            ...
            assert len(self.paths_MASK) == len(self.MASK_choices), 'MASK and mask choices have different length - {}, {}.'.format(len(self.paths_MASK), len(self.MASK_choices))
    ...
    def __getitem__(self, index):
        ...
        mask_choice = self.MASK_choices[index]
        ...
        return {'LQ': img_LQ, 'GT': img_GT, 'MASK':img_MASK, 'LQ_path': LQ_path, 'GT_path': GT_path, 'MASK_path': MASK_path, 'points': points, 'choice': mask_choice}
        # else:
        #     return {'LQ': img_LQ, 'GT': img_GT, 'MASK':img_MASK, 'LQ_path': LQ_path, 'GT_path': GT_path, 'MASK_path': MASK_path, 'points': 'others'}
